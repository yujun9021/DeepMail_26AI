...
def delete_phishing_mails():
    ...
    for idx, mail in enumerate(messages):
        combined_text = mail['subject'] + " " + mail['snippet']
    ...
    for idx in phishing_indices:
        mail = messages[idx]
        st.markdown(f"**[{idx+1}] {mail['subject']}**  \n발신자: {mail['sender']}  \n내용: {mail['snippet']}")
    ...
    if st.button("❌ 피싱 메일 삭제 진행"):
        ...
        for idx in success_indices:
            mail = messages[idx]
            msg += f"- [{idx+1}] {mail['subject']} (발신자: {mail['sender']})\n"
...
def process_user_prompt(prompt: str):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            if any(kw in prompt.lower() for kw in mail_keywords):
                if st.session_state.gmail_authenticated:
                    refresh_gmail_messages()
                    st.experimental_rerun()
